{ config, lib, ... }: {
  programs.zsh-powerlevel10k = {
    enable = true;
    theme = lib.mkMerge [
      config.programs.zsh-powerlevel10k.themes.robbyrussell
      {
        mode = "compatible";

        prompt-char.symbol = lib.mkForce "\${\${NIX_SHELL_LEVEL:+%Bλ%b}:-%B➜%b }";

        # this is the default robbyrussell git indicator, but with an
        # added pink '*' before the status if there are any changes
        # in the stash
        vcs.formatter = lib.mkForce ''
          emulate -L zsh
          if [[ -n $P9K_CONTENT ]]; then
            ...
            # if there are things in the stash, add a pink star before the status
