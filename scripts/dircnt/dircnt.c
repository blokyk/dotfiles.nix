 ...
 */
bool count(char *path, bool recursive, struct filecount *counts) {
    ...
    while((ent = readdir(dir))) {
      unsigned long subpath_len = strlen(path) + 1 + strlen(ent->d_name);
      if (subpath_len+1 > DIRCNT_PATH_MAX) {
          fprintf(stdout, "path too long (%ld) %s%c%s", subpath_len, path, PATH_SEPARATOR, ent->d_name);
          return false;
      }
...
#endif /* DEBUG */
          snprintf(subpath, subpath_len+1, "%s%c%s", path, PATH_SEPARATOR, ent->d_name);
...
/*              fprintf(stderr, "This is %s, skipping\n", ent->d_name); */
              ...
              counts->dirs++;
              if (recursive)
	              count(subpath, recursive, counts);
          }
      } else {
...
void usage() {
    fprintf(stderr, "Usage: dircnt [-r | --recursive] <dir>\n");
}
...
int main(int argc, char *argv[]) {
    ...
    if (argc <= 1) {
        ...
        // -r <dir>
        if (strcasecmp(argv[1], "-r") == 0 || strcasecmp(argv[1], "--recursive") == 0) {
            recursive = true;
            dir = argv[2];
        }
        // <dir> -r
        else if (strcasecmp(argv[2], "-r") == 0 || strcasecmp(argv[2], "--recursive") == 0) {
            recursive = true;
            dir = argv[1];
        }
        else {
